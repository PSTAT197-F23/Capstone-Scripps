  ...
  dashboardBody(
    ...
    tabItem(tabName = "bioacoustics",
            HTML("<h1><strong>CalCOFI Bioacoustics</strong></h1>
                  ...
                  <p>A faster r-CNN ResNet-50 model was built to automate the 
...
<pre><code>  #annotations saved in 'unique_annotation' variable
  ...
  original_data = np.vstack(data_matrix)</code></pre>
                  ...
                  <p>Due to the nature of the basis vectors on which the pixel values 
                  for each spectrogram are uniquely determined, we found a sound 
                  justification to safely apply a principal component projection to 
                  the training set. Singular value decomposition of the matrix was then 
                  done to separate the data's covariance matrix into three sub-matrices 
                  that together comprise the original data: U (eigenvector matrix), 
                  S (eigenvalue matrix), and T (feature matrix). Below is an example of 
...
<pre><code>  U, S, T = np.linalg.svd(original_data, full_matrices=False)
  ...
  for i in range(0, 10):
      ...
      draw_img_single(one_face)</code></pre>
                  ...
                  <br/>
                  <h3><strong>Step 3: Median Filtering and Background Subtraction
                  </strong></h3>
                  <p>With the artifacts and white noise problem frequencies spread across 
                  separate, more consistently distributed spectrograms, we performed a 
                  time-domain background subtraction on the principal spectrogram features 
                  from matrix T along with median blurring with the common kernel size of 
                  3. This would remove the artifacts and white noise from the spectrograms 
...
<pre><code>  for i in range(len(T)):
  ...
  for i in range(0, 10):
      ...
      draw_img_single(one_face)</code></pre>
                  ...
                  components as those were sufficient in creating reconstructions of the 
